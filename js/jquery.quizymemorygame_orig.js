...
(function($) {
  $.fn.quizyMemoryGame = function(options) {
    ...
    // keeps the number of cards - change directly in index.html
          ...
          if(numMatches == itemsNum/2){
            ...
            if(opts.gameSummary){
              ...
              $('div#quizy-game-summary').
                  ...
                  html(numTotalClicks/2+'<br>'+opts.textSummaryRounds);    
    ...
    var showResIcon = function(type){
      if(opts.resultIcons){
        ...
        el.delay(time).fadeIn(time/2).delay(time/2).hide("fade", time/2);
      }
    }
...
//CFM addtion - Ajax call to write game results.  See writer.php in 'write' folder for details
      $('div#quizy-game-summary').click(function(){
	    $(this).remove();
	    $.ajax({
		type: "POST",
                url: "write/writer.php", 
                data: {email: opts.subID, clicks: numTotalClicks, time: numSeconds, game: opts.gameType, grid: opts.gridSize},
                error: function(xhr, status, error){alert("Crap");}
						});	    
      });
//End CFM addition
  ...
  ****************************************************************************/
  ...
  $.fn.quizyMemoryGame.defaults = {itemWidth: 156, itemHeight: 156, itemsMargin:10, colCount:4, animType:'scroll', animSpeed:250, openDelay:2500, flipAnim:'rl', resultIcons:true, gridSize:'34', gameType:'icon', gameSummary:true, textSummaryTitle:'Summary\nClick to\nContinue', textSummaryClicks:'Clicks', textSummaryTime:'Seconds', onFinishCall:''}
